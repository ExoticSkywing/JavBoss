...
func RegisterRoutes(router gin.IRoutes) {
	...
	router.POST("/jav/items/:id/magnets/collect", collectJavMagnets)
	router.GET("/jav/items/:id", getJavItem)
	router.PATCH("/jav/items/:id/code", correctJavCode)
	router.PUT("/jav/items/:id/magnet-selection", selectJavMagnet)
	router.GET("/jav/magnet-queue", listJavDownloadQueue)
	...
	router.POST("/jav/items/:id/magnets/:candidate_id/review-decision", saveJavQualityReviewDecision)
	router.GET("/jav/items/:id/trailer", getJavTrailer)
	router.POST("/jav/items/:id/sample-images", resolveJavSampleImages)
	router.GET("/jav/items/:id/sample-images/:index", proxyJavSampleImage)
